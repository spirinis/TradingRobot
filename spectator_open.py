...
def FindShareByTicker(tiker: str) -> DataFrame:
    ...
    try:
        with Client(token_read) as client:
            ...
            try:
                shares = pd.read_csv('used_shares.csv')  # all_shares.csv
            except FileNotFoundError:
                shares = DataFrame(
                    instruments.shares(instrument_status=InstrumentStatus.INSTRUMENT_STATUS_BASE).instruments,
                    columns=['name', 'ticker', 'figi', 'lot', 'min_price_increment', 'currency', 'class_code']
                )
